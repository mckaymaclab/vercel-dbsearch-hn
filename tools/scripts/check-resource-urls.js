...
// Build regex tests. Each returns matches: { line, col, excerpt, rule }
const rules = [
  {
    rule: 'SPACE_IN_PROTOCOL_HOST',
    description: 'Spaces between protocol and host components (e.g., https://byui. idm.oclc.org)',
    regex: /(https?:\/\/[^"'\s]{0,100}?\.[a-z]{2,})(?=[^\n]* )/i, // broad pre-filter, refined below
    custom(line, idx) {
      // Find sequences like 'https://... ' then domain piece with space after a dot
  ...
  },
  {
    ...
    description: 'Residual spaced byui.idm.oclc.org variant',
    regex: /byui\s*\.\s*idm\s*\.\s*oclc\s*\.\s*org/i,
  },
];
