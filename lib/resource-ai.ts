...
function getFallbackResources(query: string): any[] {
  ...
  const scoredResources = resourceDatabase.map((resource) => {
    ...
    const searchableText = [resource.name, resource.description, ...resource.subjects, ...resource.contentTypes]
      .join(" ")
      .toLowerCase()
    ...
    if (queryLower.includes("history") && resource.subjects.some((s) => s.toLowerCase().includes("history"))) {
      score += 20
    }
    if (queryLower.includes("science") && resource.subjects.some((s) => s.toLowerCase().includes("science"))) {
      score += 20
    }
    if (queryLower.includes("business") && resource.subjects.some((s) => s.toLowerCase().includes("business"))) {
      score += 20
    }
    if (queryLower.includes("english") && resource.subjects.some((s) => s.toLowerCase().includes("english"))) {
      score += 20
    }
    if (queryLower.includes("math") && resource.subjects.some((s) => s.toLowerCase().includes("math"))) {
      score += 20
    }
    ...
    return {
      ...
      matchReason:
        matchReasons.length > 0
          ? `Smart search found this resource because it ${matchReasons.slice(0, 2).join(" and ")}.`
...
export async function findResources(query: string, apiKey?: string) {
  try {
...
export async function testGeminiConnection(apiKey?: string) {
  try {
    ...
    if (error instanceof Error && error.message.includes("API_KEY_INVALID")) {
      return {
        success: false,
        message: "Invalid API key. Please check your key and ensure the Generative AI API is enabled.",
      }
    }
    ...
    if (error instanceof Error && (error.message.includes("429") || error.message.includes("quota"))) {
      return {
        success: false,
        message: "API quota exceeded. The app will use fallback search until quota resets.",
      }
    }
...
export async function indexNewResource(resourceData: string, apiKey?: string) {
  try {
    const genAI = getGeminiClient(apiKey)
    const model = genAI.getGenerativeModel({
      model: "gemini-1.5-flash",
...
Return JSON with: id, name, description, url, subjects[], contentTypes[], accessNote, featured (false), fullText (boolean).`
    ...
    throw new Error("Failed to index new resource. Please try again later.")
  }
}
